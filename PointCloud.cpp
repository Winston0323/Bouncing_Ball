	...
	: pointSize(pointSize)
{
	...
	for (int i = 0; i < points.size(); i++) {
		...
		points[i].x = points[i].x / maxDist * 10;
		points[i].y = points[i].y / maxDist * 10;
		points[i].z = points[i].z / maxDist * 10;
...
void PointCloud::draw(const glm::mat4& view, const glm::mat4& projection, GLuint shader, const glm::vec3 viewPos, Light& light)
{
	...
	glUniform3fv(glGetUniformLocation(shader, "light.intensities"), 1, glm::value_ptr(light.intensity));
	glUniform3fv(glGetUniformLocation(shader, "viewPos"), 1, glm::value_ptr(viewPos));

	if (shadeForLight == false) {
		glUniform1i(glGetUniformLocation(shader, "shadeForLight"), 0);
	}
	if (shadeForLight == true) {
		glUniform1i(glGetUniformLocation(shader, "shadeForLight"), 1);
	}

	//glUniform1f(glGetUniformLocation(shader, "light.exponent"), );
	//setting material
	material.setMatToShader(shader);
...
void PointCloud::litTrans(glm::vec3 destination)
{
	model = glm::translate(glm::mat4(1.0f), destination)* glm::scale(glm::mat4(1.0f), glm::vec3(0.04f));
}
...
void PointCloud::setMaterial(Material material) {
	this->material = material;
}
void PointCloud::setNormalLighting(bool normal) 
{
	if (normal == true) {
		this->material.normalLighting = 1;
	}
	
	if (normal == false) {
		this->material.normalLighting = 0;
	}
...
void PointCloud::setShadeMode() 
{
	this->shadeForLight = true;
}